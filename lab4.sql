...
where asset>(select max(asset)
from branch where branchcity ="bangalore");


select distinct d.customernme
from depoitor d join bankaccount a on d.accno=a.accno
join bankaccount  on d.accno=a.accno
join loan l on l.branchname=b2.brnchname
join branch b2 on l .branchname = b2.branchname
where b1.branchcity="bangalore" and b2.branchcity="bangalore";

select d.coustomername from depositor d
join bankaccount ba on d.accno=ba.accno
join branch b on b.brnchname=b.branchname
where b.branchcity =" delhi"
group by d.coustomer having count (Distinct b.branchname)
=(select coun(*) from branch where branchcity ="delhi ");

select branchname from branch 
where assets > all(select assets from branch where branchcity ="bangalore");
